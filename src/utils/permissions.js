...
export const hasPermission = (user, permissionName) => {
  ...
  // Check if user has any roles with the required permission
  return user.roles.some(role => {
    if (typeof role === 'string') {
      // If role is just a string, assume basic permissions based on role name
      if (role === 'ROLE_ADMIN') return true;
      return false;
    }
    
    // If role is an object with permissions array
