...
import { Loader2, Edit, LayoutGrid, Armchair, X } from 'lucide-react';
import layoutService from '../../services/layoutService';

const EditLayoutDialog = ({ open, onClose, layout, onSuccess }) => {
  ...
  useEffect(() => {
    if (layout) {
      setFormData({
        ...
        driverColumn: layout.driverColumn || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await layoutService.updateLayout(layout.id, payload);
      setErrors({});
      onSuccess();
    } catch (error) {
